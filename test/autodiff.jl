...
@testset "alg:AGM" begin
    ...
    @testset "Zygote, ForwardDiff and Enzyme" begin
        ...
        # Test known derivative identities at representative nonsingular values.
        @testset for (m, ϕ, n) in zip(ms, ϕs, ns)
            ...
            @testset "Complete K" begin
                ...
                for Tret in (Const, Duplicated, DuplicatedNoNeed), Tm in (Const, Duplicated)
                    test_forward(alg.K, Tret, (enzyme_m, Tm); atol = enzyme_atol)
                end

                for Tret in (Const, Active), Tm in (Const, Active)
                    test_reverse(alg.K, Tret, (enzyme_m, Tm); atol = enzyme_atol)
                end
            end
            ...
            @testset "Complete E" begin
                ...
                enzyme_m = 0.65
                for Tret in (Const, Duplicated, DuplicatedNoNeed), Tm in (Const, Duplicated)
                    test_forward(alg.E, Tret, (enzyme_m, Tm); atol = enzyme_atol)
                end
                for Tret in (Const, Active), Tm in (Const, Active)
                    test_reverse(alg.E, Tret, (enzyme_m, Tm); atol = enzyme_atol)
                end
            end
    ...
    @testset "Zygote, ForwardDiff and Enzyme" begin
        ...
        @testset for (m, ϕ, n) in zip(ms, ϕs, ns)
            ...
            @testset "Incomplete F" begin
                ...
                ϕ_here = 0.5;
                m_here = 1.0
                @test ForwardDiff.derivative(x -> _F(ϕ_here + x, m_here), 0.0) ≈
                      1.139493927324549 atol = 1e-9 # Number retrieved from ForwardDiff and checked against finite diff
                @test ForwardDiff.derivative(x -> _F(ϕ_here, m_here + x), 0.0) ≈
                      0.025067566595210033 atol = 1e-9 # Number retrieved from ForwardDiff and checked against finite diff
                @test ForwardDiff.derivative(x -> _F(ϕ_here + x, m_here + x), 0.0) ≈
                      1.164561493919759 atol = 1e-9 # Number retrieved from ForwardDiff and checked against finite diff
            end
            ...
            @testset "CN" begin
                ...
                @test ForwardDiff.derivative(x -> alg.cn(x, x), 0.5) ≈ g1 + g2 atol = 1e-5
            end
            ...
            @testset "SN" begin
                ...
                @test ForwardDiff.derivative(x -> alg.sn(x, x), 0.5) ≈ g1 + g2 atol = 1e-5
            end
