...
@testset "Zygote and ForwardDiff" begin
    ...
    @testset for (k, ϕ, n) in zip(ks, ϕs, ns)
        ...
        @test Zygote.gradient(ϕ -> F(ϕ, m), ϕ)[1] ≈ 1 / √(1 - m*sin(ϕ)^2) atol=1e-5
        @test ForwardDiff.derivative(ϕ -> F(ϕ, m), ϕ) ≈ 1 / √(1 - m*sin(ϕ)^2) atol=1e-5
        @test Enzyme.autodiff(Reverse, ϕ -> F(ϕ, m), Active, Active(ϕ))[1][1] ≈ 1 / √(1 - m*sin(ϕ)^2) atol=1e-5
        ...
        @test Zygote.gradient(ϕ -> E(ϕ, m), ϕ)[1] ≈ √(1 - m * sin(ϕ)^2) atol=1e-5
        @test ForwardDiff.derivative(ϕ -> E(ϕ, m), ϕ) ≈ √(1 - m * sin(ϕ)^2) atol=1e-5
        @test Enzyme.autodiff(Reverse, ϕ -> E(ϕ, m), Active, Active(ϕ))[1][1] ≈ √(1 - m * sin(ϕ)^2) atol=1e-5
        ...
        @test Enzyme.autodiff(Reverse, m -> E(ϕ, m), Active, Active(m))[1][1] ≈ (E(ϕ, m) - F(ϕ, m)) / 2m atol=1e-5
