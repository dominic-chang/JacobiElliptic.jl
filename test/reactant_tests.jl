...
@testset "Reactant" begin
    ...
    @testset "Default API" begin
        ...
        incomplete_pi_r = reactant_jit(:(Pi.(0.2, $u_r, 0.5)))
        incomplete_pi_scalar_r = reactant_jit(:(Pi.(0.2, $single_u_r, 0.5)))
        incomplete_pi_neg_m_r = reactant_jit(:(Pi.(0.2, $u_r, $(k_edge_m_r[1]))))
        incomplete_pi_n_gt_one_r = reactant_jit(:(Pi.(1.7, $single_gt_one_u_r, 0.5)))
        incomplete_pi_wide_r = reactant_jit(:(Pi.(0.2, $wide_u_r, 0.5)))
        incomplete_pi_traced_n_r = reactant_jit(:(Pi.($n_r, $(us[2]), 0.5)))
        complete_pi_r = reactant_jit(:(Pi.($n_r, 0.5)))
        complete_pi_scalar_r = reactant_jit(:(Pi.($single_pi_n_r, $single_m_r)))
        complete_pi_zero_n_r = reactant_jit(:(Pi.($zero_pi_n_r, $single_m_r)))
