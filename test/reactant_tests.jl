...
@testset "Reactant" begin
    ...
    single_u_r = Reactant.to_rarray(us[3:3])
    single_gt_one_u_r = Reactant.to_rarray(us[2:2])
    single_pi_n_r = Reactant.to_rarray(pi_ns[2:2])
    zero_pi_n_r = Reactant.to_rarray(pi_ns[1:1])
    single_m_r = Reactant.to_rarray(ms[3:3])
    @testset "Default API" begin
        k_r = @jit K.(m_r)
        k_edge_r = @jit K.(k_edge_m_r)
        e_r = @jit E.(m_r)
        e_edge_r = @jit E.(k_edge_m_r)
        incomplete_e_r = @jit E.(u_r, 0.5)
        incomplete_e_gt_one_r = [@jit E(u, 1.5) for u in gt_one_us]
        incomplete_e_neg_r = [@jit E(u, -0.5) for u in us]
        incomplete_e_m_r = [@jit E(u, ms[3]) for u in us]
        incomplete_e_m_gt_one_r =
            [@jit E(u, k_edge_ms[3]) for u in gt_one_us]
        incomplete_e_m_neg_r = [@jit E(u, k_edge_ms[1]) for u in us]
        incomplete_e_gt_one_r = @jit E.(gt_one_u_r, 1.5)
        incomplete_e_neg_r = @jit E.(u_r, -0.5)
        incomplete_e_m_r = @jit E.(u_r, m_r[3])
        incomplete_e_m_gt_one_r = @jit E.(gt_one_u_r, k_edge_m_r[3])
        incomplete_e_m_neg_r = @jit E.(u_r, k_edge_m_r[1])
        incomplete_e_wide_r = @jit E.(wide_u_r, 0.5)
        incomplete_e_wide_gt_one_r = @jit E.(wide_u_r, 1.5)
        incomplete_e_wide_neg_r = @jit E.(wide_u_r, -0.5)
        f_r = @jit F.(u_r, 0.5)
        f_gt_one_r = @jit F.(gt_one_u_r, 1.5)
        f_neg_r = @jit F.(u_r, -0.5)
        f_m_r = @jit F.(u_r, m_r[3])
        f_m_gt_one_r = @jit F.(gt_one_u_r, k_edge_m_r[3])
        f_m_neg_r = @jit F.(u_r, k_edge_m_r[1])
        am_r = @jit am.(u_r, 0.5)
        am_gt_one_r = @jit am.(gt_one_u_r, 1.5)
        am_neg_r = @jit am.(u_r, -0.5)
        am_m_r = @jit am.(u_r, ms[3])
        am_m_gt_one_r = @jit am.(gt_one_u_r, k_edge_m_r[3])
        am_m_neg_r = @jit am.(u_r, k_edge_m_r[1])
        incomplete_pi_r = @jit Pi.(0.2, u_r, 0.5)
        incomplete_pi_scalar_r = @jit Pi.(0.2, single_u_r, 0.5)
        incomplete_pi_neg_m_r = @jit Pi.(0.2, u_r, k_edge_m_r[1])
        incomplete_pi_n_gt_one_r = @jit Pi.(1.7, single_gt_one_u_r, 0.5)
        incomplete_pi_wide_r = @jit Pi.(0.2, wide_u_r, 0.5)
        incomplete_pi_traced_n_r = @jit Pi.(n_r, us[2], 0.5)
        complete_pi_r = @jit Pi.(n_r, 0.5)
        complete_pi_scalar_r = @jit Pi.(single_pi_n_r, single_m_r)
        complete_pi_zero_n_r = @jit Pi.(zero_pi_n_r, single_m_r)
        complete_pi_neg_m_r = @jit Pi.(n_r, k_edge_m_r[1])
        complete_pi_gt_one_m_r = @jit Pi.(n_r, k_edge_m_r[3])
        ...
        @test Array(incomplete_pi_r) ≈ Pi.(0.2, us, 0.5)
        @test Array(incomplete_pi_scalar_r) ≈ Pi.(0.2, us[3:3], 0.5)
        @test Array(incomplete_pi_neg_m_r) ≈ Pi.(0.2, us, k_edge_ms[1])
        @test Array(incomplete_pi_n_gt_one_r) ≈ Pi.(1.7, us[2:2], 0.5)
        @test Array(incomplete_pi_wide_r) ≈ Pi.(0.2, wide_us, 0.5)
        @test Array(incomplete_pi_traced_n_r) ≈ Pi.(pi_ns, us[2], 0.5)
        @test Array(complete_pi_r) ≈ Pi.(pi_ns, 0.5)
        @test Array(complete_pi_scalar_r) ≈ Pi.(pi_ns[2:2], ms[3:3])
        @test Array(complete_pi_zero_n_r) ≈ Pi.(pi_ns[1:1], ms[3:3])
    ...
    @testset "Jacobi API" begin
        ...
        cn_r = @jit cn.(u_r, 0.5)
        dn_r = @jit dn.(u_r, 0.5)
        sn_neg_r = @jit sn.(u_r, -0.5)
        cn_neg_r = @jit cn.(u_r, -0.5)
        dn_neg_r = @jit dn.(u_r, -0.5)
        sn_gt_one_r = @jit sn.(gt_one_u_r, 1.5)
        cn_gt_one_r = @jit cn.(gt_one_u_r, 1.5)
        dn_gt_one_r = @jit dn.(gt_one_u_r, 1.5)
        sd_r = @jit sd.(u_r, 0.5)
...
#
    ...
    #    #fukushima_k_r = @jit K.(Ref(Fukushima()), m_r)
    #    carlson_f_r = @jit F.(Ref(Carlson()), u_r, 0.5)
    #    carlson_f_gt_one_r = @jit F.(Ref(Carlson()), u_r, 1.5)
    #    carlson_f_neg_r = @jit F.(Ref(Carlson()), u_r, -0.5)
    #    carlson_pi_r = @jit Pi.(Ref(Carlson()), 0.2, u_r, 0.5)
    #    #fukushima_sn_r = @jit sn.(Ref(Fukushima()), u_r, 0.5)
